...
- [x] T041 Add missing DTOs to `SalesSystem\SalesSystem.Contracts\DTOs\AllDtos.cs` — add `PurchaseInvoiceDto`, `PurchaseInvoiceLineDto`, `SalesInvoiceDto`, `SalesInvoiceLineDto`, `SalesReturnDto`, `SalesReturnItemDto`, `PurchaseReturnDto`, `PurchaseReturnItemDto`, `StockTransferDto`, `StockTransferItemDto`, `CustomerPaymentDto`, `SupplierPaymentDto` as C# records following existing DTO patterns in that file. All money fields `decimal`, all quantity fields `decimal`.
