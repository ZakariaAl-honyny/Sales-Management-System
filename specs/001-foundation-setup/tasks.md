...
- [X] T026 [US2] Create entity `SalesSystem/SalesSystem.Domain/Entities/PurchaseInvoiceItem.cs` — same pattern as SalesInvoiceItem but `UnitCost` instead of `UnitPrice`. `LineTotal = (Quantity * UnitCost) - DiscountAmount`.
...
- [X] T040 [P] [US3] Create DTO records in `SalesSystem/SalesSystem.Contracts/DTOs/`: `SalesInvoiceDto.cs` (with `IReadOnlyList<SalesInvoiceItemDto> Items`), `SalesInvoiceItemDto.cs`, `PurchaseInvoiceDto.cs` (with Items), `PurchaseInvoiceItemDto.cs`.
...
- [X] T054 [P] [US4] Create `PurchaseInvoiceConfiguration.cs` and `PurchaseInvoiceItemConfiguration.cs`. Same pattern as Sales but Supplier FK required, UnitCost instead of UnitPrice.
