...
public partial class StockTransfersControl : BaseModuleControl
{
    private readonly IStockTransferApiService _apiService;
    ...
    private BindingList<StockTransferDto> _transferList = new();

    public StockTransfersControl(
        IStockTransferApiService apiService,
    ...
    private void SetupGrid()
    {
        ...
        _grid.Columns.Add(new DataGridViewTextBoxColumn { DataPropertyName = "TransferNo", HeaderText = "رقم التحويل", Width = 150 });
        _grid.Columns.Add(new DataGridViewTextBoxColumn { DataPropertyName = "FromWarehouseName", HeaderText = "من مستودع", FillWeight = 1 });
        _grid.Columns.Add(new DataGridViewTextBoxColumn { DataPropertyName = "ToWarehouseName", HeaderText = "إلى مستودع", FillWeight = 1 });
        _grid.Columns.Add(new DataGridViewTextBoxColumn { DataPropertyName = "TransferDate", HeaderText = "التاريخ", Width = 120 });
    ...
    private async Task RefreshData()
    {
        ...
        if (result.IsSuccess)
        {
            _transferList = new BindingList<StockTransferDto>(result.Value.ToList());
            _grid.DataSource = _transferList;
        }
    ...
    {
        using var diag = new StockTransferForm(_apiService, _warehouseApi, _productApi, _notification);
