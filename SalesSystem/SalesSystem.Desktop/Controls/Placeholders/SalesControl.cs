...
public partial class SalesControl : BaseModuleControl
{
    private readonly ISalesInvoiceApiService _apiService;
    ...
    private BindingList<SalesInvoiceDto> _invoiceList = new();

    public SalesControl(
        ISalesInvoiceApiService apiService,
    ...
    private void InitializeComponent()
    {
        ...
        this.btnNew.Text = "+ فاتورة مبيعات";
    ...
    private async Task RefreshData()
    {
        ...
        if (result.IsSuccess)
        {
            _invoiceList = new BindingList<SalesInvoiceDto>(result.Value.ToList());
    ...
    {
        using var diag = new SalesInvoiceForm(_apiService, _customerApi, _productApi, _warehouseApi, _notification);
        if (diag.ShowDialog() == DialogResult.OK) _ = RefreshData();
    }

    private void ShowEditDialog(SalesInvoiceDto invoice)
    {
        using var diag = new SalesInvoiceForm(_apiService, _customerApi, _productApi, _warehouseApi, _notification, invoice);
        if (diag.ShowDialog() == DialogResult.OK) _ = RefreshData();
    }
