...
public partial class CustomersControl : BaseModuleControl
{
    ...
    private BindingList<CustomerDto> _customerList = new();
    ...
    private void InitializeComponent()
    {
        ...
        this._searchBar.Placeholder = "بحث عن عميل...";
        ...
        this._grid.CellDoubleClick += (s, e) => { if (e.RowIndex >= 0) ShowEditDialog(_customerList[e.RowIndex]); };
    ...
    private void SetupGrid()
    {
        ...
        _grid.Columns.Add(new DataGridViewTextBoxColumn { DataPropertyName = "Name", HeaderText = "اسم العميل", FillWeight = 2 });
    ...
    private async Task RefreshData()
    {
        ...
        if (result.IsSuccess)
        {
            _customerList = new BindingList<CustomerDto>(result.Value.ToList());
            _grid.DataSource = _customerList;
        }
    ...
    {
        using var diag = new CustomerDialog(_apiService, _notificationService);
        if (diag.ShowDialog() == DialogResult.OK) _ = RefreshData();
    }

    private void ShowEditDialog(CustomerDto customer)
    {
        using var diag = new CustomerDialog(_apiService, _notificationService, customer);
        if (diag.ShowDialog() == DialogResult.OK) _ = RefreshData();
    }
