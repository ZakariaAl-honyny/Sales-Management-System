...
public partial class SuppliersControl : BaseModuleControl
{
    ...
    private BindingList<SupplierDto> _supplierList = new();
    ...
    private void InitializeComponent()
    {
        ...
        this._grid.CellDoubleClick += (s, e) => { if (e.RowIndex >= 0) ShowEditDialog(_supplierList[e.RowIndex]); };
    ...
    private void SetupGrid()
    {
        ...
        _grid.Columns.Add(new DataGridViewTextBoxColumn { DataPropertyName = "Name", HeaderText = "اسم المورد", FillWeight = 2 });
        _grid.Columns.Add(new DataGridViewTextBoxColumn { DataPropertyName = "Phone", HeaderText = "الهاتف", Width = 120 });
        _grid.Columns.Add(new DataGridViewTextBoxColumn { DataPropertyName = "CurrentBalance", HeaderText = "الرصيد الحالي", Width = 100, DefaultCellStyle = new DataGridViewCellStyle { Format = "N2" } });
    ...
    private async Task RefreshData()
    {
        ...
        if (result.IsSuccess)
        {
            _supplierList = new BindingList<SupplierDto>(result.Value.ToList());
            _grid.DataSource = _supplierList;
        }
    ...
    {
        using var diag = new SupplierDialog(_apiService, _notificationService);
        if (diag.ShowDialog() == DialogResult.OK) _ = RefreshData();
    }

    private void ShowEditDialog(SupplierDto supplier)
    {
        using var diag = new SupplierDialog(_apiService, _notificationService, supplier);
        if (diag.ShowDialog() == DialogResult.OK) _ = RefreshData();
    }
