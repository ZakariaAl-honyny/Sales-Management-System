...
public partial class SalesReturnsControl : BaseModuleControl
{
    private readonly ISalesReturnApiService _apiService;
    ...
    private BindingList<SalesReturnDto> _returnList = new();

    public SalesReturnsControl(
        ISalesReturnApiService apiService,
    ...
    private void SetupGrid()
    {
        ...
        _grid.Columns.Add(new DataGridViewTextBoxColumn { DataPropertyName = "ReturnNo", HeaderText = "رقم المرتجع", Width = 150 });
        _grid.Columns.Add(new DataGridViewTextBoxColumn { DataPropertyName = "CustomerName", HeaderText = "العميل", FillWeight = 2 });
        _grid.Columns.Add(new DataGridViewTextBoxColumn { DataPropertyName = "ReturnDate", HeaderText = "التاريخ", Width = 120 });
    ...
    private async Task RefreshData()
    {
        ...
        if (result.IsSuccess)
        {
            _returnList = new BindingList<SalesReturnDto>(result.Value.ToList());
            _grid.DataSource = _returnList;
        }
    ...
    {
        using var diag = new SalesReturnForm(_apiService, _customerApi, _productApi, _warehouseApi, _notification);
