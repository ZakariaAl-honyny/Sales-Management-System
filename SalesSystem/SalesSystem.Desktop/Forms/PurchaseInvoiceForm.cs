...
public partial class PurchaseInvoiceForm : Form
{
    ...
    protected override async void OnLoad(EventArgs e)
    {
        ...
        if (_invoice != null)
        {
            ...
            btnCancel.Visible = _invoice.Status == 2;
    ...
    private async Task SaveInvoice(bool post)
    {
        ...
        var items = _lines.Select(x => new CreatePurchaseInvoiceItemRequest(x.ProductId, x.Quantity, x.UnitPrice, x.DiscountAmount, null)).ToList();
        ...
        try
        {
            ...
            if (result.IsSuccess)
            {
                if (post)
                {
                    var postResult = await _purchaseApi.PostAsync(result.Value.Id);
