...
{
    Task<Result<SystemAccountMappingDto>> GetMappingAsync(SalesSystem.Domain.Accounting.Enums.SystemAccountKey key, int? branchId = null, CancellationToken ct = default);
    Task<Result<List<SystemAccountMappingDto>>> GetAllMappingsAsync(int? branchId = null, CancellationToken ct = default);
