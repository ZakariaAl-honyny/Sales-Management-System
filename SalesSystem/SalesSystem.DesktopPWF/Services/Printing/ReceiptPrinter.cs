...
public class ReceiptPrinter : IReceiptPrinter
{
    ...
    private void Pd_PrintPage(object sender, PrintPageEventArgs e)
    {
        ...
        {
            g.DrawString($"الرقم الضريبي: {_storeInfo.TaxNumber}", smallFont, Brushes.Black, new RectangleF(x, y, width, 15), PrintHelper.RTLFormatCenter);
            y += 15;
        }
        y += 5;
        ...
        foreach (var item in _items)
        {
            ...
            if (item.Mode == 2) qStr += " (ج)";
            g.DrawString(qStr, smallFont, Brushes.Black, new RectangleF(x + width - 170, y, 50, 15), PrintHelper.RTLFormatRight);
        ...
        {
            g.DrawString($"ملاحظات: {_invoice.Notes}", smallFont, Brushes.Black, new RectangleF(x, y, width, 30), PrintHelper.RTLFormatRight);
            y += 20;
        }
