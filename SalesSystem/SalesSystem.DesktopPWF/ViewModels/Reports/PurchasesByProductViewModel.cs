...
public class PurchasesByProductViewModel : ViewModelBase
{
    ...
    private async Task ExportExcelAsync()
    {
        ...
        try
        {
            var saveFileDialog = new SaveFileDialog
            {
                ...
                FileName = $"PurchasesByProduct_{DateTime.Now:yyyyMMdd_HHmm}.xlsx"
            };
            ...
            if (saveFileDialog.ShowDialog() == true)
            {
                using (var workbook = new XLWorkbook())
                {
                    ...
                    worksheet.Cell(Entries.Count + 2, 3).Value = TotalCost;
                    worksheet.Cell(Entries.Count + 2, 4).Value = TotalCost;
                ...
                await D.ShowInfoAsync("نجاح", "تم تصدير تقرير المشتريات حسب المنتج إلى Excel بنجاح");
            }
        }
        catch (Exception ex)
        {
            LogSystemError("فشل في تصدير تقرير المشتريات حسب المنتج إلى Excel", "PurchasesByProductViewModel.ExportToExcel", ex);
    ...
    private async Task ExportPdfAsync()
    {
        ...
        try
        {
            ...
            await PdfExportService.ExportToPdfAsync("المشتريات حسب المنتج", dataTable, TotalCost,
                $"PurchasesByProduct_{DateTime.Now:yyyyMMdd_HHmm}.pdf");
        }
        catch (Exception ex)
        {
            LogSystemError("فشل في تصدير تقرير المشتريات حسب المنتج إلى PDF", "PurchasesByProductViewModel.ExportPdf", ex);
            await D.ShowErrorAsync("خطأ في تصدير الملف", "حدث خطأ غير متوقع أثناء تصدير الملف. يرجى المحاولة مرة أخرى.");
        }
    }
