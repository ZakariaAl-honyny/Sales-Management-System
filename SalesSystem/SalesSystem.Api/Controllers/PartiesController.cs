...
public class PartiesController : ControllerBase
{
    ...
    /// <summary>
    /// Gets all parties (customers and suppliers).
    /// </summary>
    [HttpGet]
    ...
    /// <summary>
    /// Creates a new party (customer or supplier) with auto-created account.
    /// </summary>
    [HttpPost]
    ...
    [HttpPut("{id:int}")]
    [Authorize(Policy = "ManagerAndAbove")]
    public async Task<IActionResult> Update(int id, [FromBody] UpdatePartyRequest request, CancellationToken ct)
    {
        var userIdStr = User.FindFirst(ClaimTypes.NameIdentifier)?.Value;
        if (!int.TryParse(userIdStr, out var userId))
            return Unauthorized();
    ...
    /// <summary>
    /// Deactivates (soft-deletes) a party.
    /// </summary>
    [HttpDelete("{id:int}")]
    [Authorize(Policy = "ManagerAndAbove")]
