...
public partial class MainForm : Form
{
        ...
        IEventBus eventBus)
    {
        ...
        // تفعيل وضع اللغة العربية (من اليمين لليسار) للنافذة بالكامل
        this.RightToLeft = RightToLeft.Yes;
        this.RightToLeftLayout = true;
        
        // إجبار القائمة الجانبية على الالتصاق بالجهة اليمنى
        pnlSidebar.Dock = DockStyle.Right;
        
        SalesSystem.Desktop.Helpers.ThemeHelper.ApplyMainFormStyle(this);
    }
    ...
    private Button CreateNavButton(NavigationItem item)
    {
        var btn = new Button
        {
            ...
            TextAlign = ContentAlignment.MiddleRight, // Changed to Right for Arabic
            Margin = new Padding(0, 0, 0, 8),
            Cursor = Cursors.Hand,
            Tag = item,
            Font = new Font("Segoe UI", 10F, FontStyle.Bold),
            Padding = new Padding(0, 0, 15, 0) // Add right padding for the text
        };
    ...
    private void NavigateTo(Type screenType)
    {
        ...
        {
            Log.Error(ex, "حدث خطأ أثناء التنقل إلى {ScreenType}", screenType.Name);
        }
    }
