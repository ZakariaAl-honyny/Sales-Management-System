...
| Test Case | Description | Expected Result |
|-----------|-------------|-----------------|
| TC-21-001 | SalesInvoice: Created without InvoiceNo | `SalesInvoice.Create()` no longer requires invoiceNo parameter; entity.Id is auto-generated by DB |
| TC-21-002 | SalesInvoiceService: GetByNumberAsync removed | No `GetByNumberAsync` method exists on `ISalesService` or `SalesService` |
| TC-21-003 | SalesInvoice List: Search uses int.TryParse + Id | Entering number searches by `Id == parsedInt` instead of `InvoiceNo.Contains(search)` |
| TC-21-004 | SalesReportDto: No InvoiceNo field | DTO uses `int Id` as invoice identifier, not `string InvoiceNo` |
| TC-21-005 | InvoicePrintDto: Uses int Id | Print DTO uses `int Id` instead of `string InvoiceNo` for document identification |
